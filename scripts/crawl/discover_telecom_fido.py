"""Discover Fido plan URLs.

Category: telecom-fido, Collection: ca_telecom_fido.
Network: Rogers (Flanker tier)

Usage:
    uv run python scripts/crawl/discover_fido.py
    uv run python scripts/crawl/discover_fido.py --supplemental-only
"""
...
SUPPLEMENTAL_URLS = [
    ...
    "https://www.fido.ca/phones",
    "https://www.fido.ca/mobility/network-coverage-map",
...
async def run_bfs_discovery(headless=True):
    ...
    for i, url in enumerate(BFS_SEEDS):
        ...
        def url_filter(u):
            return urlparse(u).netloc in ("www.fido.ca", "fido.ca")
        try:
