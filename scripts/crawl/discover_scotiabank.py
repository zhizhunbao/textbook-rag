"""Discover Scotiabank personal banking URLs.

BFS crawl from Scotiabank newcomer/banking/credit-card pages.
Category: bank-scotiabank, Collection: ca_bank_scotiabank.

Usage:
    uv run python scripts/crawl/discover_scotiabank.py
    uv run python scripts/crawl/discover_scotiabank.py --dry-run
    uv run python scripts/crawl/discover_scotiabank.py --supplemental-only
"""
import asyncio
import json
import sys
...
# ── Config ──
PERSONA = "bank-scotiabank"
MANIFEST_PATH = Path(__file__).resolve().parents[2] / "data" / "crawled_web" / PERSONA / "manifest.json"

# Phase 1: BFS seeds
BFS_SEEDS = [
    # Newcomer 专属页面（StartRight Program）
    "https://www.scotiabank.com/ca/en/personal/bank-accounts/newcomers-to-canada.html",
    # 个人银行账户
    "https://www.scotiabank.com/ca/en/personal/bank-accounts.html",
    # 信用卡
...
_BASE = "https://www.scotiabank.com/ca/en/personal"
SUPPLEMENTAL_URLS = [
    # ── Newcomer ──
    f"{_BASE}/bank-accounts/newcomers-to-canada.html",
    f"{_BASE}/bank-accounts/newcomers-to-canada/startright-program.html",
    f"{_BASE}/bank-accounts/newcomers-to-canada/international-students.html",
    # ── Bank Accounts ──
    f"{_BASE}/bank-accounts.html",
    f"{_BASE}/bank-accounts/chequing-accounts.html",
    f"{_BASE}/bank-accounts/savings-accounts.html",
    f"{_BASE}/bank-accounts/preferred-package.html",
    # ── Credit Cards ──
    ...
    f"{_BASE}/credit-cards/scene-visa.html",
    # ── Students ──
    f"{_BASE}/bank-accounts/students.html",
    f"{_BASE}/bank-accounts/students/student-banking-advantage-plan.html",
    # ── Fees ──
    f"{_BASE}/bank-accounts/bank-service-fees.html",
]
...
async def run_bfs_discovery(headless: bool = True):
    """Discover Scotiabank pages via crawl4ai BFS."""
    ...
    for i, url in enumerate(BFS_SEEDS):
        ...
        def url_filter(u: str) -> bool:
            parsed = urlparse(u)
            return (
                parsed.netloc in ("www.scotiabank.com", "scotiabank.com")
                and "/ca/en/personal/" in parsed.path
            )
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discover Scotiabank personal banking URLs")
