"""Discover Virgin Plus plan URLs.

Category: telecom-virginplus, Collection: ca_telecom_virginplus.
Network: Bell (Flanker tier)

Usage:
    uv run python scripts/crawl/discover_virginplus.py
    uv run python scripts/crawl/discover_virginplus.py --supplemental-only
"""
...
SUPPLEMENTAL_URLS = [
    ...
    "https://www.virginplus.ca/en/phones/bring-your-own-device",
    "https://www.virginplus.ca/en/phones",
...
async def run_bfs_discovery(headless=True):
    ...
    for i, url in enumerate(BFS_SEEDS):
        ...
        def url_filter(u):
            return urlparse(u).netloc in ("www.virginplus.ca", "virginplus.ca")
        try:
