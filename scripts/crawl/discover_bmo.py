"""Discover BMO personal banking URLs.

BFS crawl from BMO newcomer/banking/credit-card pages.
Category: bank-bmo, Collection: ca_bank_bmo.

Usage:
    uv run python scripts/crawl/discover_bmo.py
    uv run python scripts/crawl/discover_bmo.py --dry-run
    uv run python scripts/crawl/discover_bmo.py --supplemental-only
"""
import asyncio
import json
import sys
...
# Phase 1: BFS seeds
BFS_SEEDS = [
    # Newcomer 专属页面（BMO NewStart Program）
    ...
    # 学生
    "https://www.bmo.com/main/personal/students/",
]
BFS_DEPTH = 2
BFS_MAX_PAGES = 500

# Phase 1.5: Supplemental URLs — 确保关键页面不被遗漏
_BASE = "https://www.bmo.com/main/personal"
SUPPLEMENTAL_URLS = [
    # ── Newcomer ──
    f"{_BASE}/newcomers-to-canada/",
    f"{_BASE}/newcomers-to-canada/newstart-program/",
    # ── Bank Accounts ──
    f"{_BASE}/bank-accounts/chequing-accounts/",
    f"{_BASE}/bank-accounts/savings-accounts/",
    f"{_BASE}/bank-accounts/no-fee-banking/",
    # ── Credit Cards ──
    f"{_BASE}/credit-cards/",
    f"{_BASE}/credit-cards/no-annual-fee/",
    f"{_BASE}/credit-cards/cashback/",
    f"{_BASE}/credit-cards/rewards/",
    f"{_BASE}/credit-cards/low-interest/",
    f"{_BASE}/credit-cards/no-fee-mastercard/",
    # ── Students ──
    f"{_BASE}/students/",
    f"{_BASE}/students/student-banking/",
    # ── Fees ──
    f"{_BASE}/bank-accounts/account-fees/",
]
...
async def run_bfs_discovery(headless: bool = True):
    """Discover BMO pages via crawl4ai BFS."""
    ...
    for i, url in enumerate(BFS_SEEDS):
        ...
        # URL filter: 只保留 bmo.com/main/personal/ 路径下的页面
        def url_filter(u: str) -> bool:
            parsed = urlparse(u)
            return (
                parsed.netloc in ("www.bmo.com", "bmo.com")
                and parsed.path.startswith("/main/personal")
            )
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description="Discover BMO personal banking URLs")
