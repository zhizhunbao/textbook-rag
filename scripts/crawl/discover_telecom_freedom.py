"""Discover Freedom Mobile plan URLs.

Category: telecom-freedom, Collection: ca_telecom_freedom.
Network: Freedom/Shaw (Independent)

Usage:
    uv run python scripts/crawl/discover_freedom.py
    uv run python scripts/crawl/discover_freedom.py --supplemental-only
"""
...
SUPPLEMENTAL_URLS = [
    ...
    "https://www.freedommobile.ca/en-CA/support/international-roaming",
    "https://www.freedommobile.ca/en-CA/support/international-calling",
...
async def run_bfs_discovery(headless=True):
    ...
    for i, url in enumerate(BFS_SEEDS):
        ...
        def url_filter(u):
            return urlparse(u).netloc in ("www.freedommobile.ca", "freedommobile.ca")
        try:
