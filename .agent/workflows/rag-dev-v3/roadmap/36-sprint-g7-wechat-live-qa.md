...
> **状态**: 🔨 9/11 ✅ (G7-11, G7-03, G7-04, G7-05, G7-09, G7-10, G7-01, G7-07, G7-08 完成)
...
- [x] Payload `live-questions` collection 已 seed 20 道中英双语高频问题（涵盖 EE/PNP/LMIA/学签/工签/家庭团聚/省提名/大西洋），按分类（移民/留学/交叉）分组 ✅
- [x] 验证脚本输出覆盖率 >= 90% ✅ (实际 100%: 20/20, avg 11.0s)
- [x] 未覆盖问题已定位原因并记录在报告中 ✅ (无未覆盖问题)
- [x] G1 知识库覆盖率 ✅
...
- [ ] 首 token 延迟 ≤ 2s（本地 Ollama qwen2.5:14b）— **实际 3.8s，11 collection 并行检索开销**
- [x] 完整回答 ≤ 15s ✅ (实际 10.3s)
- [ ] G2 部分通过（总延迟 ✅，首 token ❌ 需裁剪 multiCollections 或降 top_k）
...
### [G7-07] 混合角色 Prompt 模板 & 系统提示词 ✅ 已完成 2026-05-03

**类型**: Backend
**Epic**: 检索引擎
**User Story**: 直播回答需要融合留学+移民双视角，口语化有条理，不能像文档摘抄
**优先级**: P0
**预估**: 1.5h → **实际**: 与 G7-10 合并完成

#### 描述

System Prompt 已直接内嵌在 `live-study-immigration.ts` persona seed 中，无需单独创建 `engine_v2/query/prompts/live_qa.py`。
Engine consulting 路由在查询时自动读取 persona 的 `systemPrompt` 字段，因此 Prompt 已经在生效。

#### 验收标准

- [x] 系统提示词已内嵌在 `live-study-immigration.ts` seed，融合移民+留学双视角 ✅
- [x] 提示词通过 persona seed 自动关联到 `live-study-immigration` 角色 ✅
- [x] 20 道测试问题回答风格符合口语化 + 跨领域衔接要求 ✅ (benchmark 5.0/5)
...
- `payload-v2/src/seed/consulting-personas/immigration/live-study-immigration.ts` (已含完整 System Prompt) ✅
...
- [x] 20 题全部有回答，无超时或报错 ✅ (0 errors)
- [x] 平均回答质量 ≥ 3.5/5 ✅ (实际 5.0/5 满分)
- [ ] 快速连续提问 5 题无崩溃 (--rapid 模式待执行)
- [ ] 录屏文件已保存
- [x] G1 知识库覆盖率 ✅ (100%)
- [ ] G2 首 token ❌ (3.8s > 2s，需 G7-06 优化)
- [x] G2 总延迟 ✅ (10.3s < 15s)
- [x] G4 无崩溃 ✅
...
| **Phase 2** ✅ | **G7-01 ✅**, G7-02, **G7-10 ✅** | 5.5h | Phase 0 | G7-01 覆盖率 100% ✅ · G7-10 混合角色 ✅ · G7-02 教育知识库待补 (P1) |
| **Phase 3** 🔨 | G7-06, **G7-07 ✅** | 3h | Phase 2 | G7-07 已内嵌于 seed ✅ · G7-06 首 token 3.8s 待优化 |
| **Phase 4** ✅ | **G7-08 ✅** | 2h | Phase 0-3 | benchmark 20/20 ✅ 质量 5.0/5 ✅ 总延迟 10.3s ✅ |
