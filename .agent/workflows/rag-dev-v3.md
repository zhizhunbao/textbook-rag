...
> **ConsultRAG 是全球跨境生活一站式 AI 智能顾问平台**：7 大类 × 28 个精准角色，用户选择目标国家和专家角色，上传私有文档，获得基于专业知识库 + 私有文档的双库联合检索回答，每条回答附带引用溯源和质量评分。

**核心架构**: 7 大类 (`education / immigration / settlement / healthcare / finance / career / legal`) × 28 角色 (P0: 10 / P1: 10 / P2: 8)

**核心价值主张**：
- 🎭 **28 角色精准顾问** — 每个角色对应独立知识库 + 专属系统提示词，窄领域高精度
- 📄 **私有文档隔离** — 用户上传的 PDF 仅自己可见，与角色知识库联合检索
- 🔍 **可溯源回答** — 每条回答标注来源章节、检索路径、质量评分
- 🌍 **全球复制** — 一套架构适配全国家，仅更换知识库即可上线新国家
...
> **任何形式的 fallback 都是 bug。错误必须立即抛出异常，不允许吞掉。**

| 模式 | ❌ 禁止 | ✅ 正确 |
|------|---------|--------|
| 吞异常 | `except: pass` / `except: return None` | `except SomeError: raise` 或不捕获 |
| 默认值兜底 | `value = result or "default"` (掩盖 None) | 如果 result 不该是 None，就 `assert result is not None` |
| 静默降级 | `try: use_A() except: use_B()` | 直接 `use_A()`，失败就崩 |
| 空返回 | `if not data: return []` (掩盖上游错误) | `if not data: raise ValueError("upstream returned empty")` |
| 日志代替异常 | `logger.warning("failed"); return default` | `raise RuntimeError("failed: ...")` |

**原则**: 开发阶段宁可崩溃也不要静默错误。如果代码路径不应该走到某个分支，那就 `raise`，不要 `return default`。
