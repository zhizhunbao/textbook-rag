...
| **跨段一致性** | 结论/预告/表格之间无矛盾、无冗余、与下期内容对齐 | = 100% |
...
> ⚠️ **结论、预告、表格、下期内容必须逻辑自洽，否则观众会觉得前后矛盾。**

| 检测项 | 定义 | 示例 | 修复 |
| ------ | ---- | ---- | ---- |
| **结论vs预告矛盾** | 结论的行动建议与预告承诺的下期内容逻辑冲突 | 结论说"开了也亏钱"，预告说"下期教你开哪家" → 观众困惑：亏钱为什么还要开？ | 结论必须给出开的理由（如"少亏就是赚"），预告顺着结论往下接 |
| **跨段数据冗余** | 结论和预告重复引用同一组数据或同一句话 | 结论说"下一期详细讲五大银行怎么选"，预告又说"下一期详细拆解五大银行" → 同一件事说两遍 | 结论只管总结行动，预告只管钩子引导，各司其职 |
| **下期内容对齐** | 结论/预告暗示的下期主题与实际下期内容不一致 | 结论说"还需要别的工具"（暗示GIC/投资），但下期实际讲HISA对比 → 误导预期 | 结论/预告的措辞必须精确匹配下期的实际主题 |
| **跨主体对比越界** | 某 slide 引用了后面还没讲到的主体，或与总结矛盾 | RBC slide 说"跟BMO和CIBC一样"（还没讲到）；CIBC slide 说"利率最高"但总结说Scotiabank最高 | 每个 slide 只能引用前面已讲过的主体，且不能与总结冲突 |

### 检查步骤

1. **提取结论台词** — 找到 `[argument] 结论` 的台词
2. **提取预告台词** — 找到 `[preview]` 的台词
3. **逐句对比** — 是否有语义重复？是否有逻辑矛盾？
4. **核对下期主题** — 结论/预告承诺的内容是否与系列规划中的下一集一致？
5. **检查表格与台词** — 结论表格的"结果"列是否与台词自洽？
6. **跨主体对比扫描** — 每个 slide 台词中提到的其他主体，是否已在前面的 slide 讲过？是否与总结 slide 矛盾？
