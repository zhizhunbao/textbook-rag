...
- 每个数据页有 `**引用**` + `**来源**`
- **表格引用 1:1** — 表格行数 = 引用条数
- 每页有 `**台词**`（解读型风格，支持自由段落，无需强制手动换行）
- **台词中判断句 + 翻译句占比 50% 以上**
- **标点间 ≤18 中文字，严禁使用破折号「—」「——」（TTS 质检铁律）。长段台词在合成时会自动按句末标点（。！？!?）进行智能安全拆分**
- **禁止重复台词（铁律 6）**
- 信息点 ≤ 3，slides ≤ 12（不含引用来源页）
- **多Slide简化模式** — 当内容幻灯片较多（总 slides > 12 页或内容页 > 8 页）时，**必须启用简化模式**以缩短总播放时长、优化完播率，防止听觉疲劳：
  - **去掉长举例**：完全删去下半段的冗长故事/罗翔例子段落，仅保留上半段核心内容。
  - **有机口语化**：不使用机械的“首先/其次/另外”列表连词，改为单段流畅、高级的口语化解读。
  - **1:1音画数值融合**：在口语解读中自然念出表格上的核心数据与变量（如 100%、365天、6万）。
  - **精简高节奏过渡**：在每页台词末尾增加一句 10-12 字的极简过渡句/过渡问句（如“那么，落地第一年该怎么存？”），自然连贯地引出下一页。
  - **数值字元分层**：描述性数字（如第一年、三年）用汉字，显示参数（如 100%、33,810、6万）用阿拉伯数字。
...
> 💡 **智能断句**：`synthesize.py` 内部已集成智能断句正则，支持大段落台词输入，会自动按句末标点（。！？!?）进行呼吸切口分拆，显著降低手写台词折行的心智负担，并平滑连贯音调。
